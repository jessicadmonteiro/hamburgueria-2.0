...
export const AuthProductsProvider = ({children}: iProductsContextProps) =>
{
    ...
    const valueTotal = cart.reduce((accumulator:number, currentValue) => accumulator + currentValue.price * currentValue.count, 0);
    ...
    function IncrementCount (product:iCart){
        const updatedArray = cart.map((item) => item.id === product.id ? {...item, count: item.count + 1} : item);
    ...
    <ContexProducts.Provider value={{ products, productsFilter, search, setSearch, cart, setCart, addItemToCart, modalOpen, setModalOpen, removeCart, valueTotal, RemoverAll, IncrementCount, DecrementCount}}>
        {children}
